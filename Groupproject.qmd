...
"Inverse Document Frequency" (IDF) measures the informativeness of any term, *t*. An IDF value will be very low for "stop words," or common words like "is" or "a" because they aren't very informative on their own. 

Thus, the TF-IDF score is a measure of originality of term *t* calculated by comparing the number of times term *t* appears in a document with the number of documents term *t* appears in. A higher TF-IDF score indicates the importance and relevance of a term. Lower importance terms will have scores closer to 0. TF-IDF was selected for analysis for a quick summary of priorities/keywords across survey items and to easily compare and contrast priorities and concerns between roles. 
...
# Analysis: Word and Document-topic probabilities
```{r, echo = FALSE,results = 'hide', fig.keep = 'all'}
set.seed(42)
...
```{r, echo = FALSE,results = 'hide', fig.keep = 'all'}
# Predictive modeling uses a slightly different data format than tidytext.
# We want each row in the data to correspond with the observations we are using for predictions. 
# In this case,we want one row per Federalist paper. 
# remove stop words with anti_join() and the stop_words tibble
...
recipe( ~ text, data = tidy_allquestions) %>%
  # tokenize the text
  step_tokenize(text) %>%
  # remove stop works
  step_stopwords(text) %>%
...
negative <- c("ineffective_communication", "climate_barriers")
eos_modeling <- allquestions_text_1 %>%
  mutate(
