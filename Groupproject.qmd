...
Topic Modeling is a method for unsupervised classification of text analysis. Even when we do not know what should we analyze from the text data, this model can find some hidden groups of topics. In this paper, we first conducted the analysis of the question by question. Next, we employ this topic modeling to find the most important predictor of negative and positive climate in the school workplaces across the survey data. For instance, some frequently used words such as "email" and "meeting" are important words but must have multiple meanings both negative and positive. Topic modeling can find the hidden meaning by annotating the document based on the predicted topic to find the truly impactful predictors. 

LDA is one of the methods of topic modeling. LDA aims to find the topic of the document based on the words that belong to it. This method disregards grammar and other words.  
...
In the coefficients analysis, the word "lack", "barrier", and "inconsistent" have the biggest impact for predicting the negative climate in the school workplace. Regarding the predictors for positive climate, "meeting" has both meanings negative and positive, yet we found that it is more likely to be used in the positive context from this analysis. "improve" sounds a good predictor for the positive climate; however, it has only a relatively small impact on the positive climate.

For the variable importance, "lack" is the best word in our survey data to predict negative climate. the importance significantly decreases from the second variable. "enough" and "student" are following, which are also predictor for the negative climate.

As a whole, negative climate predictors got higher figures than predictors for positive climate. This indicates that negative climate factors are not such variety in school workplaces, which means it is easier to set a goal to eliminate the negative factors than to set a goal for creating good climate.

The Random Forest is the better model compared to Lasso model in terms of RSME.
